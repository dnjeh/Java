...
public class SelectRGB extends JDialog implements AdjustmentListener, ActionListener {
	...
	public SelectRGB(JTextArea chatArea) {
		...
		scroll_r = new JScrollBar(JScrollBar.HORIZONTAL, 126, 1, 0, 256);
		scroll_g = new JScrollBar(JScrollBar.HORIZONTAL, 126, 1, 0, 256);
		scroll_b = new JScrollBar(JScrollBar.HORIZONTAL, 126, 1, 0, 256);
	...
	public void adjustmentValueChanged(AdjustmentEvent e) {
		...
		if(e.getSource()==scroll_r) {
			temp = scroll_r.getValue();
			label_r.setText(Integer.toString(temp));
		}
		else if(e.getSource()==scroll_g) {
			temp = scroll_g.getValue();
			label_g.setText(Integer.toString(temp));
		}
		else if(e.getSource()==scroll_b) {
			temp = scroll_b.getValue();
			label_b.setText(Integer.toString(temp));
		}
